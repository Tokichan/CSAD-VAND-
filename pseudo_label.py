...
def remove_background(background,masks):
    ...
    for mask in masks:
        ...
        # cv2.waitKey(0)
        if np.sum(np.logical_and(background,mask))/np.sum(mask!=0) < 0.95:
            result.append(mask)
    return result
...
def split_masks_by_connected_component(masks):
    ...
    for i,mask in enumerate(masks):
        ...
        for j in range(num_labels):
            ...
            if w*h < mask_uint.shape[0]*mask_uint.shape[1]*0.95:# and area>0.01*mask_uint.shape[0]*mask_uint.shape[1]:
                #pass
...
def intersect_ratio(mask1,mask2):
    ...
    return ratio
def intersect_ratio_order(mask1,mask2):
    # ratio of mask1 in mask2
    intersection = np.logical_and(mask1,mask2)
    if intersection.sum() == 0:
        return 0
...
def filter_masks_by_grounding_mask(grounding_mask,masks):
    new_mask = list()
    for mask in masks:
        # cv2.imshow(f"{intersect_ratio_order(mask,grounding_mask)}",mask)
        # cv2.waitKey(0)
        if np.sum(np.logical_and(grounding_mask,mask))/np.sum(mask!=0) > 0.9:
...
def filter_by_combine(masks):
    ...
    if len(wait_masks) != 0:
        for mask in wait_masks:
            ...
            # show_mask = np.hstack([np.where(combine_masks,255,0).astype(np.uint8),mask])
            # show_mask = cv2.resize(show_mask,[512,256])
            # cv2.imshow(f"{ratio}",show_mask)
            # cv2.waitKey(0)
            if ratio < 0.9:
                combine_masks = np.logical_or(combine_masks,mask)
                result_masks.append(mask)
...
def grounding_segmentation(image,model,predictor,config):
    """
    config = {
        "box_threshold": 0.8,
        "text_threshold": 0.8,
    ...
    g_image = transforms.ToPILImage()(image)
    transform = T.Compose([
        T.RandomResize([800], max_size=1333),
        T.ToTensor(),
        T.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225]),
    ])
    g_image, _ = transform(g_image, None)
    boxes_filt, pred_phrases = get_grounding_output(
        model, g_image, text_prompt, box_threshold, text_threshold, device="cuda"
    )
    ...
    # numpy_image = (image*255).permute(1,2,0).numpy().astype(np.uint8)
    numpy_image = np.array(transforms.ToPILImage()(image))
    ...
    transformed_boxes = predictor.transform.apply_boxes_torch(boxes_filt, (256,256)).to(device)
...
def segmentation(img,mask_generator,grounding_result,config):
    use_grounding_filter = config['use_grounding_filter']
    no_sam = config['no_sam']
    if no_sam:

        refined_masks = grounding_result['masks']
    ...
    raw_splited_masks = split_masks_by_connected_component(raw_masks)
    raw_splited_masks = sorted(raw_splited_masks.copy(),key=lambda x:np.sum(x),reverse=True)[:255]

    # cv2.imshow("color_raw",color_masks(raw_splited_masks))
    # cv2.waitKey(0)
    
    # fliter mask that are not inside grounding mask
    if use_grounding_filter:
        grounding_mask = grounding_result['masks']
        grounding_mask = np.where(grounding_mask!=0,255,0).astype(np.uint8)
        # cv2.imshow("grounding_mask",grounding_mask)
        # cv2.waitKey(0)
        splited_masks = filter_masks_by_grounding_mask(grounding_mask,raw_splited_masks)
    # cv2.imshow("color_refined",color_masks(splited_masks))
    # cv2.waitKey(0)
    ...
    grounding_background = grounding_result['background']
    ...
    # cv2.imshow("background",background)
    # cv2.waitKey(0)
    # cv2.imshow("gbackground",grounding_background)
    # cv2.waitKey(0)
    # cv2.imshow("color_all",color_masks(splited_masks))
    # cv2.waitKey(0)

    splited_masks = merge_masks(filter_by_combine(splited_masks))
    splited_masks = split_masks_from_one_mask(splited_masks)
    # cv2.imshow("color_refined",color_masks(splited_masks))
    # cv2.waitKey(0)

    return merge_masks(splited_masks)
...
class ClassHistogram():
    ...
    def extract_component_features(self):
        ...
        for image,mask in tqdm.tqdm(zip(self.images,self.masks),desc="extracting component features..."):
            ...
            for feature_name in ['area','color','position','cnn_shape','cnn_image']:
        ...
        # np.save(f"{self.info_path}/all_component_feats.npy",self.all_component_feats)
        # self.mean = np.mean(self.all_component_feats,axis=0)
        # self.std = np.std(self.all_component_feats,axis=0)
        # self.all_component_feats = (self.all_component_feats - self.mean) / self.std
    
    def cluster_component_features(self,feature_list=['area','color','position','cnn_shape','cnn_image']):
        result_feature = list()
...
if __name__ == "__main__":
    ...
    category = "screw_bag"
    config = read_config(f"C:/Users/kev30/Desktop/anomaly/EfficientAD-res/configs/class_histogram/{category}.yaml")
    """
    metal bolts . metal hex nuts . metal washers . metal screws . zip-lock bag . screw . ring . metal circle . bag . bolt . container . nut . package . plastic . screw . tool .

    almond . apple . banana . cereal . container . fill . food . fruit . grain . granola . mixture . nut . oatmeal . oats . orange . plastic . raisin . seed . topping . tray .

    alcohol . apple juice . beverage . bottle . liquor . glass bottle . juice . lemonade . liquid . olive oil . orange juice . yellow banana . bottle . glass bottle . glass jar . jug . juice . liquid . milk beverage . bottle . cherry . condiment . liquor . glass bottle . honey . juice . liquid . maple syrup . sauce . syrup . tomato sauce .

    attach . cable . connect . connector . hook . electric outlet . plug . pole . socket . wire .

    box . case . container . fill . needle . pin . plastic . screw . tool . tray . yellow . pushpin . drawing pin . thumbtack .
    
    """
    image_paths = glob.glob(f"{config['project_root']}/datasets/mvtec_loco_anomaly_detection/{config['category']}/train/good/*.png")
    # feature_extractor = PDN_M(padding=True)
    # ckpt = torch.load("C:/Users/kev30/Desktop/anomaly/EfficientAD/ckpt/teacher_medium_new.pth")
    # feature_extractor.load_state_dict(ckpt)
    feature_extractor = timm.create_model('wide_resnet50_2.tv2_in1k'
                                          ,pretrained=True,
                                          features_only=True,
                                          out_indices=[4])
